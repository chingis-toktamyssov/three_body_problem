...
// ------------------------------------------------------------
// Gravitational force of body2 on body1
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// Compute velocities + accelerations for RK4
// ------------------------------------------------------------
...
void rk4(sunBody& body1, sunBody& body2, sunBody& body3, double dt) {
    ...
    // k2
    t1 = body1;
    t1.position += k1[0] * (dt * 0.5);
    t1.velocity += k1[1] * (dt * 0.5);

    t2 = body2;
    t2.position += k1[2] * (dt * 0.5);
    t2.velocity += k1[3] * (dt * 0.5);

    t3 = body3;
    t3.position += k1[4] * (dt * 0.5);
    t3.velocity += k1[5] * (dt * 0.5);

    accVelo(t1, t2, t3, k2);

    // k3
    t1 = body1;
    t1.position += k2[0] * (dt * 0.5);
    t1.velocity += k2[1] * (dt * 0.5);

    t2 = body2;
    t2.position += k2[2] * (dt * 0.5);
    t2.velocity += k2[3] * (dt * 0.5);

    t3 = body3;
    t3.position += k2[4] * (dt * 0.5);
    t3.velocity += k2[5] * (dt * 0.5);
